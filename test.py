...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq, params: Params):
    ...
    for i in prog_bar:
        ...
        scores, file_content = eval_bs_batch(batch, model, vocab, pack_seq=params.pack_seq,
                                             beam_size=params.beam_size,  # beam大小
                                             min_out_len=params.min_out_len,  # 最小输出
                                             max_out_len=params.max_out_len,  # 最大输出
...
if __name__ == "__main__":
    ...
    print("Load train_status from:{}".format(args.model))
    train_status = torch.load(args.model)
    print("Load params...")
    p = train_status['params']
    print("Load best model:{}".format('%s.%02d.pt' % (p.model_path_prefix, train_status['best_epoch_so_far'])))
