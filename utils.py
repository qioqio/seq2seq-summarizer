...
class Example(NamedTuple):
    # Example继承自NamedTuple类型，可以访问其内部属性
    src: List[str]
    tgt: List[str]
...
class OOVDict(object):
    ...
    def __init__(self, base_oov_idx):
        ...
        self.base_oov_idx = base_oov_idx  # oov词典的索引接着vocab继续
        self.ext_vocab_size = base_oov_idx  # see 2017:vocabulary + all words appearing in the source document

    def add_word(self, idx_in_batch, word) -> int:
        key = (idx_in_batch, word)  # (0,"xxxxx")
...
class Dataset(object):
    ...
    def __init__(self, filename: str, tokenize: Callable = simple_tokenizer, max_src_len: int = None,
        ...
        self.pairs = []
        self.src_len = 0  # the max length of source
        self.tgt_len = 0  # the max length of target

        if filename.endswith('.gz'):
            open = gzip.open

        # r:reading t:text model（default mode）
        with open(filename, 'rt', encoding='utf-8') as f:
            for i, line in enumerate(f):
                pair = line.strip().split('\t')  # 使用tab作为分隔符。
                if len(pair) != 2:
    ...
    def build_vocab(self, vocab_size: int = None, src: bool = True, tgt: bool = True,
        ...
        filename, _ = os.path.splitext(self.filename)  # filename=./data/cnndm  _=.gz
        if vocab_size:
            filename += ".%d" % vocab_size
        filename += '.vocab'
        # ./data/cnndm.30000.vocab
        if os.path.isfile(filename):
            ...
            # 按照词表大小和词频调整词表
            # TODO
            vocab.trim(vocab_size=vocab_size, min_freq=1)
                ...
                for i, example in enumerate(examples):
                    if src_vocab:
                        for j, word in enumerate(example.src):
                            # 取出词语对应ID
                            idx = src_vocab[word]
                            # 如果在vocab中没有找到该词语（可能是因为vocab截断处理被去掉了）
                            if ext_vocab and idx == src_vocab.UNK:
                                idx = oov_dict.add_word(i, word)  # (0,"xxxxx")
                            src_tensor[j, i] = idx
...
def save_data(p, path):
    """
    save params into the experiment dir
    :param path: experiment dir
    :return: None
    """
    f = open(path + "params.txt", "w")
    f.write("vocab_size:" + str(p.vocab_size) + "\n")
    ...
    f.write("tie_embed:" + str(p.tie_embed) + "\n")
    f.write("\n")
    f.write("enc_attn_cover:" + str(p.enc_attn_cover) + "\n")
    f.write("cover_func:" + str(p.cover_func) + "\n")
    f.write("cover_loss:" + str(p.cover_loss) + "\n")
    f.write("show_cover_loss:" + str(p.show_cover_loss) + "\n")
    f.write("\n")
    f.write("enc_rnn_dropout:" + str(p.enc_rnn_dropout) + "\n")
    f.write("dec_in_dropout:" + str(p.dec_in_dropout) + "\n")
    f.write("dec_rnn_dropout:" + str(p.dec_rnn_dropout) + "\n")
    f.write("dec_out_dropout:" + str(p.dec_out_dropout) + "\n")
    f.write("\n")
    f.write("optimizer:" + str(p.optimizer) + "\n")
    f.write("lr:" + str(p.lr) + "\n")
    f.write("adagrad_accumulator:" + str(p.adagrad_accumulator) + "\n")
    f.write("lr_decay_step:" + str(p.lr_decay_step) + "\n")
    f.write("lr_decay:" + str(p.lr_decay) + "\n")
    f.write("batch_size:" + str(p.batch_size) + "\n")
    f.write("\n")
    f.write("choose_model:" + str(p.choose_model) + "\n")
    f.write("\n")
    f.write("n_batches:" + str(p.n_batches) + "\n")
    f.write("val_batch_size:" + str(p.val_batch_size) + "\n")
    f.write("n_val_batches:" + str(p.n_val_batches) + "\n")
    f.write("n_epochs:" + str(p.n_epochs) + "\n")
    f.write("pack_seq:" + str(p.pack_seq) + "\n")
    f.write("forcing_ratio:" + str(p.forcing_ratio) + "\n")
    f.write("partial_forcing:" + str(p.partial_forcing) + "\n")
    f.write("forcing_decay_type:" + str(p.forcing_decay_type) + "\n")
    f.write("forcing_decay:" + str(p.forcing_decay) + "\n")
    f.write("sample:" + str(p.sample) + "\n")
    f.write("grad_norm:" + str(p.grad_norm) + "\n")
    f.write("rl_ratio:" + str(p.rl_ratio) + "\n")
    f.write("rl_ratio_power:" + str(p.rl_ratio_power) + "\n")
    f.write("rl_start_epoch:" + str(p.rl_start_epoch) + "\n")
    f.write("\n")
    f.write("embed_file:" + str(p.embed_file) + "\n")
    f.write("val_data_path:" + str(p.val_data_path) + "\n")
    f.write("\n")
    f.write("max_src_len:" + str(p.max_src_len) + "\n")
    f.write("max_tgt_len:" + str(p.max_tgt_len) + "\n")
    f.write("truncate_src:" + str(p.truncate_src) + "\n")
    f.write("truncate_tgt:" + str(p.truncate_tgt) + "\n")
    f.write("\n")
    f.write("model_path_prefix:" + str(p.model_path_prefix) + "\n")
    f.write("keep_every_epoch:" + str(p.keep_every_epoch) + "\n")
    f.write("\n")
    f.write("beam_size:" + str(p.beam_size) + "\n")
    f.write("min_out_len:" + str(p.min_out_len) + "\n")
    f.write("max_out_len:" + str(p.max_out_len) + "\n")
    f.write("out_len_in_words:" + str(p.out_len_in_words) + "\n")
    f.write("test_data_path:" + str(p.test_data_path) + "\n")
